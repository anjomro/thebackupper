...
cat key.bin
echo -ne "\n"
openssl enc -aes-256-cbc -iter 100000  -in $1 -out $1.enc -pass file:./key.bin
echo "Encrypted $1, stored as $1.enc!"
mv $1 ORIGINAL$1
openssl rsautl -encrypt -inkey ~/.ssh/id_rsa_publickey -pubin -in key.bin -out key.bin.enc
rm key.bin
